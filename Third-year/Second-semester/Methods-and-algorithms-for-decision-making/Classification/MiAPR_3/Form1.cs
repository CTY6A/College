...
namespace MiAPR_3
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            double PC1, PC2, p1, p2;
            double falseAlarmError = 0, missingDetectingError = 0, totalClassificationError;
            ...
            if ((PC1 > 1) || (PC2 > 1))
                ...
                else
                {
                    ...
                    for (x = 0; x < pointsCount; x++)
                    {
                        ...
                        bufferedGraphics.Graphics.FillRectangle(new SolidBrush(Color.DarkBlue),
                            (int)(x), pictureBox1.Height - (int) (p1 * PC1 * scaleMode), 2, 2);
                    }
                    for (x = 0; x < pointsCount; x++)
                    {
                        ...
                        bufferedGraphics.Graphics.FillRectangle(new SolidBrush(Color.Red),
                             (int)(x), pictureBox1.Height - (int) (p2 * PC2 * scaleMode), 2, 5);
                    }
                    ...
                    borderX = x;
                    while (x < pictureBox1.Width + offset)
                    {
                        p1 = PC1 * Math.Exp(-0.5 * Math.Pow((x - mu1) / sigma1, 2)) /
                            (sigma1 * Math.Sqrt(2 * Math.PI));
                        missingDetectingError += p1 * Eps;
                        x += Eps;
                    }

                    bufferedGraphics.Graphics.DrawLine(new Pen(Color.Green, 1),
