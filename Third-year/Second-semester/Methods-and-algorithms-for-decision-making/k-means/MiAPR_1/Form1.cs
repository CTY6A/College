...
namespace MiAPR_1_VS
{
    ...
    {
        private const int MIN_POINTS_COUNT = 1000;
        ...
        private const int KERNELS_WIDTH = 12;
        private const int BORDER_WIDTH = 5;
        private const int MIN_AVERAGE_SQUARE_DISTANCE = 5000;                                                                                                                                                                                                                                                                                                                          private const int PRECISION1 = 300;    //Точность 1
        ...
        private void ClearForm()
        {
            ...
            dataGridView1.Columns[0].Width = dataGridView1.Columns[1].Width = dataGridView1.Width / 2 - 12;
            dataGridView1.Rows[0].Cells[0].Value = "Класс:";
            dataGridView1.Rows[0].Cells[1].Value = "Количество:";
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            while (is_class_change)
            {
                ...
                {
                    double MinAverageSquareDistance = MIN_AVERAGE_SQUARE_DISTANCE;
                    int number = -1;
                ...
                for (int i = 0; i < points_count; i++)
                    bufferedGraphics.Graphics.FillRectangle(new SolidBrush(Colors[Points[i].Class]),
                        Points[i].x, Points[i].y, POINTS_WIDTH, POINTS_WIDTH);
                for (int i = 0; i < class_count; i++)
                    bufferedGraphics.Graphics.FillEllipse(new SolidBrush(Color.FromArgb(Colors[NewKernels[i].Class].ToArgb() ^ 0xffffff)),
                        NewKernels[i].x, NewKernels[i].y, KERNELS_WIDTH, KERNELS_WIDTH);
                bufferedGraphics.Render();
            }
