...
var
  ...
  i, j, CurCLI, MaxCLI, NumOfCase, NumOfElseIf, CL, CLsmall: Integer;
...
begin
  ...
  begin
    ...
    case Str[1] of
      ...
      ','           : begin
                        if Str[1] = '}' then
                        begin
                          if CurCLI > MaxCLI then
                            MaxCLI := CurCLI;
                          if NumOfCase = 0 then
                            Dec(CurCLI)
                          else
                          begin
                            CurCLI := CurCLI - NumOfCase;
                            NumOfCase := 0;
                            Dec(CL)
                          end;
                        end;
      ...
      'a'..'z', '_' : begin
                        ...
                        if (Word = 'andfor') or ('andwhile' = Word) or (Word = 'case') or ('else' = Word) or (Word = 'break') or (Word = 'end') or (Word = 'do') then
                        else
                          if (Word = 'and') or ('or' = Word) or (Word = 'xor') or ('continue' = Word) or ('return' = Word) or (Word = 'as') or (Word = 'begin') then
  ...
  for i := 1 to SGOperands.RowCount - 1 do
    SGOperands.Cells[0,i]:=IntToStr(i);

  for i := 1 to SGOperators.RowCount - 1 do
    SGOperators.Cells[0,i]:=IntToStr(i);
  ...
  SG3Lines.Cells[1,0]:='n='+inttostr(SGOperators.RowCount-2)+'+'+inttostr(SGOperands.RowCount-2)+'='+inttostr(SGOperators.RowCount-2+SGOperands.RowCount-2);
  ...
  SG3Lines.Cells[1,3]:=IntToStr(CL);
  SG3Lines.Cells[1,4]:=FloatToStrF(CL / (CL + CLsmall),ffFixed,1,2);
  SG3Lines.Cells[1,5]:=IntToStr(MaxCLI - 1);
...
begin
  ...
  SG3Lines.Cells[0,3]:='Абсолютная сложность программы CL';
  SG3Lines.Cells[0,4]:='Относительная сложность программы cl';
  SG3Lines.Cells[0,5]:='Максимальный уровень вложенности условного оператора CLI';
end;
