...
namespace Bank.Models
{
    public class AddCreditViewModel
    {
        ...
        [RegularExpression(@"\d{8}", ErrorMessage = "Incorrect format for number of credit - 8 numbers")]
        ...
        public AddCreditViewModel(CreditInfo program, DateTime date)
        {
            Name = program.Name;
            Duration = program.Duration;
            CreditInfoId = program.Id;
            Currency = program.Currency.ToString();
            Persent = program.Persent.ToString();
            StartDate = date;
            //EndDate = StartDate.AddDays(Duration).Date;
            EndDate = StartDate.AddMonths(Duration).Date;
            Type = program.Differantal ? "Differentiated" : "Annuity";
        }
