...
namespace lab1
{
    public partial class MainForm : Form
    {
        ...
        private void buttonStart_Click(object sender, EventArgs e)
        {
            ...
            var disp = x.Sum(t => Math.Pow(t - matOzh, 2)) / x.Count;
            ...
            var period = pos[1] - pos[0];
            var pos0 = 0;
            for (int i = 0; i + period < x.Count; i++)
            {
                if (Math.Abs(x[i] - x[i + period]) < epsilon)
                {
                    pos0 = i;
                    break;
                }
            }
            richTextBox.Text += @"Длина периода = " + period + '\n';
            richTextBox.Text += @"Длина участка апериодичности = " + (period + pos0) + '\n';
        }
